...
Mỗi dịch vụ AWS đảm nhiệm một vai trò cụ thể: S3 private, CloudFront và WAF tạo lớp web edge; ALB và Auto Scaling phân phối backend trên hai EC2 với EBS mã hóa; RDS PostgreSQL Multi-AZ lưu dữ liệu; VPC/Security Group áp luồng ALB → backend → database; IAM Role cấp quyền giám sát; CloudWatch thu log, metric và trạng thái alarm.
...
5. Chạy FastAPI dưới dạng `systemd` trên các instance ASG, đồng thời giám sát ALB, ASG, EC2 và RDS bằng CloudWatch.
...
| Backend và dữ liệu | ALB → ASG với hai FastAPI instance; telemetry và lệnh lưu trong RDS PostgreSQL Multi-AZ |
| Frontend | Bản build React + Vite trong S3 private, phân phối qua CloudFront OAC với WAF monitoring |
| Giao tiếp | Trình duyệt HTTPS tới CloudFront; `/api/*` tới ALB; thiết bị HTTP trực tiếp tới ALB; polling, thực thi và ACK |
| Giám sát | Log backend, widget ALB/ASG/EC2/RDS và tám CloudWatch alarm |
...
| :--- | :--- |
| Hạ tầng AWS | Bằng chứng CloudFront/WAF/S3 private, ALB/target group/ASG/EC2/EBS, RDS Multi-AZ, VPC/Security Group, IAM và CloudWatch |
...
| Giám sát | Log backend, widget metric ALB/ASG/EC2/RDS và tám CloudWatch alarm |
| Kiểm thử | Ma trận T01–T14 với toàn bộ kết quả được ghi nhận là Đạt và có bằng chứng liên kết |
...
| S07 | Giám sát | Log backend, widget ALB/ASG/EC2/RDS và tám cấu hình alarm đều hiển thị | Đạt |
| S08 | Khả năng tái tạo và an toàn | T01–T14 đều được ghi nhận là Đạt, tài liệu bàn giao đã có và credential không xuất hiện trong repository | Đạt |
